...
function Buscar_Seccion2($asi_cod,$mod_id,$reg_id,$esp_id,$coh_id,$pen_top){
  $pac=$this->Buscar_Pacade();
  $this->pac_id=$pac->pac_id;
  $resp=$this->OperacionCualquiera("SELECT A.ase_id AS 'ase_id', A.sec_id AS 'sec_id', B.sec_nom AS 'sec_nom', A.ase_cma AS 'ase_cma' FROM asigna_seccio A, seccio B WHERE asi_cod='$asi_cod' AND mod_id='$mod_id' AND reg_id='$reg_id' AND esp_id='$esp_id' AND coh_id='$coh_id' AND pen_top='$pen_top' AND pac_id='$this->pac_id' AND ase_sta='1' AND A.sec_id=B.sec_id AND B.sec_sta='1' GROUP BY ase_id");
  return $resp;
}
//******************************************************************
...
/*  echo "<script>alert('UPDATE detins SET det_sta=1,det_obs=INCLUSION DE ASIGNATURA, obs_id=12 WHERE det_id=$ultimo_id');</script>";*/
...
//if($inc_sec[$i]=="")$inc_sec[$i]=0;
  if($inc_obs[$i]!="")
	$resp=$this->OperacionCualquiera("INSERT INTO hisest (his_fso, his_sol, his_sac, his_sap, his_sta, ci_est, det_id, ci_emp, pro_id, his_fap, his_des) VALUES ('$his_fso', '$this->his_sol', '0', '$inc_sec[$i]', '$proc', '$this->ci_est', '$ultimo_id', '$this->ci_emp', '$proceso[1]', '$hoy', '$inc_obs[$i]')");
  else	
...
/*  echo "<script>alert('INSERT INTO hisest (his_fso, his_sol, his_sap, his_sta, ci_est, det_id, ci_emp, pro_id, his_fap, his_des) VALUES ($his_fso, $this->his_sol, $cam_sec[$i], $proc, $this->ci_est, $ultimo_id, $this->ci_emp, $proceso[2], $hoy, $cam_obs[$i])');</script>";*/
	  $resp=$this->OperacionCualquiera("INSERT INTO hisest (his_fso, his_sol, his_sac, his_sap, his_sta, ci_est, ci_emp, pro_id, his_fap, his_des) VALUES ('$his_fso', '$this->his_sol', '$cam_sec_act[$i]', '$cam_sec[$i]', '$proc', '$this->ci_est', '$this->ci_emp', '$proceso[2]', '$hoy', '$cam_obs[$i]')");
	}
	else{
/*  echo "<script>alert('INSERT INTO hisest (his_fso, his_sol, his_sap, his_sta, ci_est, det_id, ci_emp, pro_id, his_fap, his_des) VALUES ($his_fso, $this->his_sol, $cam_sec[$i], $proc, $this->ci_est, $ultimo_id, $this->ci_emp, $proceso[2], $hoy, $cam_obs[$i])');</script>";*/
	if($cam_obs[$i]!="")
	  $resp=$this->OperacionCualquiera("INSERT INTO hisest (his_fso, his_sol, his_sac, his_sap, his_sta, ci_est, det_id, ci_emp, pro_id, his_fap, his_des) VALUES ('$his_fso', '$this->his_sol', '$cam_sec_act[$i]', '$cam_sec[$i]', '$proc', '$this->ci_est', '$ultimo_id', '$this->ci_emp', '$proceso[2]', '$hoy', '$cam_obs[$i]')");
	else
      $resp=$this->OperacionCualquiera("INSERT INTO hisest (his_fso, his_sol, his_sac, his_sap, his_sta, ci_est, det_id, ci_emp, pro_id, his_fap) VALUES ('$his_fso', '$this->his_sol', '$cam_sec_act[$i]', '$cam_sec[$i]', '$proc', '$this->ci_est', '$ultimo_id', '$this->ci_emp', '$proceso[2]', '$hoy')");
	}
    $hisest=$this->filas_afectadas($resp);
	$i++;
	$sumar=$sumar+$i+320;
...
/*  echo "<script>alert('SELECT det_id FROM detins WHERE mod_id=$mod_id AND reg_id=$reg_id AND esp_id=$esp_id AND coh_id=$coh_id AND pen_top=$pen_top AND ci_est=$ci AND det_sta=1');</script>";*/
   $resultado=$this->OperacionCualquiera("SELECT det_id FROM detins WHERE mod_id='$mod_id' AND reg_id='$reg_id' AND esp_id='$esp_id' AND coh_id='$coh_id' AND pen_top='$pen_top' AND ci_est='$ci' AND det_sta='1'");
...
function Buscar_Cond_Asig($asi_cod){
    ...
    $this->pen_top=$_SESSION[pen_top];
	$this->ci_est=$_SESSION[ci_est];
/*if(($asi_cod=='MAT-31114') && $this->ci=='20792053'){  echo "<script>alert('SELECT det_con FROM detins WHERE mod_id=$this->mod_id AND reg_id=$this->reg_id AND esp_id=$this->esp_id AND coh_id=$this->coh_id AND pen_top=$this->pen_top AND asi_cod=$asi_cod AND det_sta=1 AND ci_est=$this->ci AND det_con=1');</script>";}*/
    $resul=$this->OperacionCualquiera("SELECT det_con FROM detins WHERE mod_id='$this->mod_id' AND reg_id='$this->reg_id' AND esp_id='$this->esp_id' AND coh_id='$this->coh_id' AND pen_top='$this->pen_top' AND asi_cod='$asi_cod' AND det_sta='1' AND ci_est='$this->ci_est' AND det_con='1'");
	return $resul;
}
//******************************************************************
...
/*	  if($this->ci=='23548519' && $asi_cod=='CIV-30314') echo "<script>alert('ENTRE A REVISAR CADA REQUISITO SI ESTA APROBADO $array_req->asi_cod_req');</script>";*/
...
/*if($this->ci=='23548519' && $asi_cod=='CIV-30314') echo "<script>alert('COMPARAR CANT REQUISITOS $cant_requi==$cant_req_apro');</script>";*/
...
/*if($this->ci=='23548519' && $asi_cod=='CIV-30314') echo "<script>alert('CO $cant_co_req_tr>0');</script>";*/
...
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('CONDICION DE APROBADA $apro');</script>";*/
		if($apro==0){ //ASIGNATURA NO APROBADA O NO VISTA
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('ASIGNATURA NO APROBADA O NO VISTA $array_oferta->asi_cod');</script>";*/
...
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('ENTRE A REVISAR CADA REQUISITO SI ESTA APROBADO $array_req->asi_cod_req');</script>";*/
		    $requi_con=$this->Buscar_Cond_Asig($array_req->asi_cod_req);
		    $cant_req_t=$this->NumFilasCualquiera($requi_con);
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('REQUISITO APROBADO $cant_req_t');</script>";*/
			if($cant_req_t>0){
			  $cant_req_apr++;
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('aprobo el requisito $cant_req_apr');</script>";*/
			}
			else{
		      if($array_req->req_cuc!=""){
/*echo "<script>alert('ENTRE A COMPARAR UC 1');</script>";*/
			    $resul_UC=$this->ConsultarCualquiera($this->Suma_UC_Aprobadas());
/*echo "<script>alert('ENTRE A COMPARAR UC 2 $resul_UC->suma_uc_apr>=$array_req->req_cuc');</script>";*/
                if($resul_UC->suma_uc_apr>=$array_req->req_cuc && $resul_UC->suma_uc_apr!='')
				  $cant_req_apr++;
			  }
			}
		  }
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('COMPARAR CANT REQUISITOS $cant_req==$cant_req_apr');</script>";*/
		  if($cant_req==$cant_req_apr){
		    ...
		    $cant_co_req_apr=0;
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('CANTIDAD DE CO-REQUISITOS $cant_co_req');</script>";*/
		    while($array_co_req=$this->ConsultarCualquiera($co_requi)){//REVISAR CADA CO-REQUISITO SI ESTA APROBADO O OFERTADO.
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('ENTRE A REVISAR CADA CO-REQUISITO SI ESTA APROBADO O OFERTADO $array_co_req->asi_cod_req');</script>";*/
		      $co_requi_con=$this->Buscar_Cond_Asig($array_co_req->asi_cod_req);
		      $cant_co_req_t=$this->NumFilasCualquiera($co_requi_con);
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('CO $cant_co_req_t>0');</script>";*/
			  if($cant_co_req_t>0){//REVISAR CADA CO-REQUISITO SI ESTA APROBADO
			    $cant_co_req_apr++;
			  }
			  else{ //REVISAR CADA CO-REQUISITO SI ESTA OFERTADO Y LA PUEDE VER
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('REVISAR CADA CO-REQUISITO SI ESTA OFERTADO Y LA PUEDE VER');</script>";*/
...
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('ESTA OFERTADA PERO HAY QUE REVISAR SI EL CORREQUISITO LO PUEDE VER');</script>";*/
...
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('COMPARAR CO-REQUISITO $cant_co_req==$cant_co_req_apr');</script>";*/
			if($cant_co_req==$cant_co_req_apr){
/*if($this->ci=='20792053' && $array_oferta->asi_cod=='MAT-31214') echo "<script>alert('COMPARAR SE ES SIN REQUISITO $cant_req_apr==0 && $cant_co_req_apr==0 && $cant_req_UC==0');</script>";*/
		      if($cant_req_apr==0 && $cant_co_req_apr==0){
		        ...
		        $cant_sin_req_apr=0;
/*if($this->ci=='20792053' && $array_oferta->asi_cod=='MAT-31214') echo "<script>alert('CANTIDAD SIN REQUISITOS $cant_sin_req');</script>";*/
			    while($array_sin_req=$this->ConsultarCualquiera($asi_sin_req)){//REVISAR CADA ASIGNATURA SIN REQUISITO SI ESTA APROBADO
/*if($this->ci=='20792053' && $array_oferta->asi_cod=='MAT-31214') echo "<script>alert('ENTRE A REVISAR CADA CADA ASIGNATURA SIN REQUISITO SI ESTA APROBADO $array_sin_req->asi_cod');</script>";*/
			      $sin_requi_con=$this->Buscar_Cond_Asig($array_sin_req->asi_cod);
		          $cant_sin_req_t=$this->NumFilasCualquiera($sin_requi_con);
/*if($this->ci=='20792053' && $array_oferta->asi_cod=='MAT-31214') echo "<script>alert('ENTRE A REVISAR CADA CADA ASIGNATURA SIN REQUISITO SI ESTA APROBADO $array_sin_req->asi_cod $cant_sin_req_t>0');</script>";*/
			      if($cant_sin_req_t>0){
			        $cant_sin_req_apr++;
			      }
			    }
/*if($this->ci=='20792053' && $array_oferta->asi_cod=='MAT-31214') echo "<script>alert('COMPARAR SIN REQUISITO $cant_sin_req==$cant_sin_req_apr');</script>";*/
...
/*if($this->ci=='23548519' && $array_oferta->asi_cod=='CIV-30144') echo "<script>alert('LISTA_DE_OFERTA: $lis_asi_ofer1');</script>";*/
			    if($lis_asi_ofer==''){
				  $lis_asi_ofer1="".$array_oferta->asi_cod."";
				  $lis_asi_ofer="'".$array_oferta->asi_cod."'";
				}
...
/*echo "<script>alert('SELECT * FROM asigna WHERE asi_cod IN() AND mod_id=$mod_id AND reg_id=$reg_id AND esp_id=$esp_id AND coh_id=$coh_id AND pen_top=$pen_top ORDER BY asi_mod,asi_nom');</script>";*/
